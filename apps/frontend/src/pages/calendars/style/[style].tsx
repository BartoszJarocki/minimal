...
const VALID_STYLES: PSEOStyle[] = ["simple", "grid"];
...
export default function StylePage({ content, currentYear, style }: Props) {
  const url = `https://useminimal.com/calendars/style/${style}`;
  const date = DateTime.now().set({ year: currentYear });
  const calendarStyle: CalendarStyle = style === "grid" ? "frame" : "default";
    ...
    <>
      ...
      <BreadcrumbSchema
        items={[
          ...
          { name: style === "grid" ? "Grid" : "Simple", url },
        ]}
      />
      ...
      <Container>
        <main className="pb-24">
          ...
          <section className="mt-8 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Link
                href={`/calendars/style/${style === "grid" ? "simple" : "grid"}`}
                className="text-sm underline"
              >
                {style === "grid" ? "Simple Calendar" : "Grid Calendar"}
              </Link>
...
export const getStaticPaths: GetStaticPaths = async () => {
  return {
    paths: VALID_STYLES.map((style) => ({ params: { style } })),
    fallback: false,
  };
};

export const getStaticProps: GetStaticProps<Props> = async ({ params }) => {
  const style = params?.style as PSEOStyle;
