...
// One template, three personalities — each headline set in the theme's own
// typeface so the OG card actually demonstrates the theme it's selling.
const THEME_OG: Record<
  ThemeSlug,
  {
    ...
    tagline: string;
    font: string;
    // Tailwind classes (satori is happier with these than inline numeric
    // fontWeight, which can crash the render).
    weightClass: string;
...
> = {
  editorial: {
    ...
    tagline: "Refined. Sans-serif. Magazine.",
    font: "Geist",
  ...
  mono: {
    label: "MONO",
    title: "MONO CALENDAR",
    tagline: "Tabular. Uppercase. Terminal.",
    font: "GeistMono",
  ...
  pixel: {
    ...
    tagline: "Square. Display. Deliberately retro.",
    font: "GeistPixel",
...
export default async function OpenGraphImageHandler(req: NextRequest) {
  const [GeistBold, GeistBlack, GeistMonoBold, GeistPixel] = await Promise.all([
    bold,
    black,
    monoBold,
    pixel,
  ...
  const fonts = [
    ...
    { name: "GeistPixel", data: GeistPixel, weight: 500 as const },
  ];
  ...
  // type=calendar - one template, headline set in the theme's own typeface
  if (type === "calendar") {
      ...
      (
        ...
        >
          ...
          <div tw="flex flex-col">
            ...
            <div
              tw={`text-7xl leading-none mt-4 ${cfg.weightClass} ${cfg.trackingClass}`}
              style={{ fontFamily: cfg.font }}
            >
              {cfg.title}
            </div>
            <div
              tw="text-2xl mt-7 text-[#1f1f1f]/70"
              style={{ fontFamily: "GeistMono" }}
            >
              {cfg.tagline}
            </div>
